 ...
 */
contract CuratorFeeFollowModule is FeeModuleBase, FollowValidatorFollowModuleBase {
    ...
    function getProfileData(uint256 profileId) external view returns (ProfileData memory) {
        ...
        /**
     * @dev curate a post by "staking" a fee - fee currently transfered to profile owner
     *  1. Charging a fee
     */
    function curateDispatch(
        address follower,
        uint256 profileId,
        DataTypes.CuratedPostData calldata data
    ) external onlyHub {
        // only profile followers can curate posts
        if (!this.isFollowing(profileId, follower, 0))
            revert NotFollower();
        
        // Charge the staking fee for curating a post
        uint256 amount = _dataByProfile[profileId].amount;
        address currency = _dataByProfile[profileId].currency;
        ...
        // Create post
        // Profile owner must have set this FollowModule as a dispatcher
        ILensHub(HUB).post(data.postData);
